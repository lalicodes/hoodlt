...
class ComputeHydrogenBonds:
    ...
    """
    def __init__(self, file_name, num_frames=None, donor_list=None, proton_list=None, acceptor_list=None, rmin_list=[3.0], distance=['d-a'], angles=[[180, 150]], centers=['H'], skip=1, eps=1e-3):
        """
        ...
        self.theta_cut = np.deg2rad(angles)
        ...
        for ind in range(self.num_hb_types):
            if distance[ind] == 'p-a':
                hb_tags, all_tags = self._get_hbonds(self.donor_list[ind], self.proton_list[ind], self.acceptor_list[ind], self.r_cut[ind], self.theta_cut[ind], centers[ind])
            if distance[ind] == 'd-a':
                hb_tags, all_tags = self._get_hbonds_da(self.donor_list[ind], self.proton_list[ind], self.acceptor_list[ind], self.r_cut[ind], self.theta_cut[ind], centers[ind])
    ...
    def _get_hbonds(self, donor, proton, acceptor, r_cut, theta, center):
        """
        ...
        for ind, ind_frame in enumerate(range(self.ini_frame, self.end_frame, self.skip)):
            snap = self.sys.read_frame(ind_frame)
            ...
            if acceptor == 'OW':
                hb_pa = np.nonzero(np.logical_and(dist_all <= r_cut, dist_all > dist_d_p))
            else:
                hb_pa = np.nonzero(dist_all <= r_cut)
            ...
            ind_hb = self._ind_hb(box, pos, a_tags, d_tags, p_tags, center)
    ...
    def _get_hbonds_da(self, donor, proton, acceptor, r_cut, theta, center):
        """
        calculates the number of hydrogen bonds given a donor, proton and acceptor
        according to the criteria of Berndt et. al. (1993)
        ...
        for ind, ind_frame in enumerate(range(self.ini_frame, self.end_frame, self.skip)):
            ...
            a_tags = copy.deepcopy(alla_tags)
            d_tags = copy.deepcopy(alld_tags)
            # check all acceptor-donor distances that satisfy the hb condition
            dist_all = box.compute_all_distances(pos[d_tags], pos[a_tags])
            hb_da = np.nonzero(dist_all <= r_cut)
            d_tags = d_tags[0][hb_da[0]]
            a_tags = a_tags[0][hb_da[1]]
            hb_pd = np.nonzero(box.compute_all_distances(pos[d_tags], pos[p_tags]) < dist_d_p) # will pick up both hydrogens
            p_tags = p_tags[0][hb_pd[1]]
            # those are the tags (acceptor, donor, proton) defining the hydrogen bonds
            if self.min_cos==-1 and self.max_cos==1:
                hb_tags[ind] = (a_tags, d_tags, p_tags[0::2], p_tags[1::2])
            else:
                ind_hb1 = self._ind_hb(box, pos, a_tags, d_tags, p_tags[0::2], center=center)
                ind_hb2 = self._ind_hb(box, pos, a_tags, d_tags, p_tags[1::2], center=center)
                p_tags = np.array(list(p_tags[0::2][ind_hb1]) + list(p_tags[1::2][ind_hb2]))
                a_tags = np.array(list(a_tags[ind_hb1]) + list(a_tags[ind_hb2]))
                d_tags = np.array(list(d_tags[ind_hb1]) + list(d_tags[ind_hb2]))
                hb_tags[ind] = (a_tags, d_tags, p_tags)

        return hb_tags, (alla_tags, alld_tags, allp_tags)


    def _ind_hb(self, box, pos, a_tags, d_tags, p_tags, center='H'):
        ...
        if center == 'OW':
            cos_val = -(dist3*dist3-dist1*dist1-dist2*dist2)/(2*dist1*dist2)
