...
:synopsis: Defines the unit test for the hex_2d_lat_Mixt class: hexagonal lattice

.. moduleauthor:: Xun Zha <xzha@iastate.edu> November 2018
"""

import unittest
import hoodlt.Lattices.Lat_dim2_1.hex_2d_lat_Mixt as Hexagonal
import numpy as np
...
class TestLatChain(unittest.TestCase):
    ...
    def setUp(self):
        self.l_val = [3, 4, 1]
        self.a_nn = 1
        self.config = Hexagonal.LatHexagonal(self.l_val, self.a_nn)
    ...
    def test_a_vector(self):
        ...
        self.assertEqual(((self.config.a_vector[1]-np.array([0.5, 0.5*np.sqrt(3), 0.0])*self.a_nn) == 0).all(), True)
    ...
    def test_gl(self):
        self.assertEqual(self.config.g_l(), 2/np.sqrt(3))
